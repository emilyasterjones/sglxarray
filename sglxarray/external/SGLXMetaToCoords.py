...
def XYCoord20(meta, elecInd, bankMask, shankInd, showPlot):
    ...
    if showPlot:
        if pType == "21":
            ...
            # plot all positions
            marker_style = dict(
                c="w", edgecolor="k", linestyle="None", marker="s", s=20
            )
            plt.scatter(elecPos[:, 0], elecPos[:, 1], **marker_style)

            # plot selected positions
            marker_style = dict(
                c="b", edgecolor="b", linestyle="None", marker="s", s=15
            )
            plt.scatter(xCoord, yCoord, **marker_style)
...
def CoordsToText(
    chans, xCoord, yCoord, connected, shankInd, shankSep, baseName, savePath, buildPath
):
